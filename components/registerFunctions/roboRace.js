...
const roboRace = async (req, res) => {
    try {
        const { teamName, utr, leader, members } = req.body;
        console.log("[ROBO RACE] Registration request:", { teamName, leaderName: leader?.name });
        ...
        const registrationId = await teamInsertionQuery("Robo Race", teamName, utr, leader, members);
        if (!registrationId) {
        ...
        res.status(201).json({
            success: true,
            message: "Robo Race registered successfully",
            data: {
        ...
        handleRegistrationError(error, res, "Robo Race");
    }
}

module.exports = roboRace;
